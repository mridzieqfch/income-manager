<?php 
// File: backend/crypto/insert_crypto.php
...
$conn = connectDB();
$stmt = $conn->prepare("INSERT INTO transactions (user_id, type, tanggal, pair_or_symbol, lot_or_amount, profit_loss, keterangan) VALUES (?, 'crypto', ?, ?, ?, ?, ?)");
$stmt->bind_param("issdds", $user_id, $data->tanggal, $data->pair_or_symbol, $data->lot_or_amount, $data->profit_loss, $keterangan);

if ($stmt->execute()) {
    send_response(201, "success", "Transaksi Crypto berhasil disimpan.");
} else {
