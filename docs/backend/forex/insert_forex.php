<?php 
// File: backend/forex/insert_forex.php
...
$conn = connectDB();
$stmt = $conn->prepare("INSERT INTO transactions (user_id, type, tanggal, pair_or_symbol, lot_or_amount, profit_loss, keterangan) VALUES (?, 'forex', ?, ?, ?, ?, ?)");
$stmt->bind_param("issdds", $user_id, $data->tanggal, $data->pair_or_symbol, $data->lot_or_amount, $data->profit_loss, $keterangan);

if ($stmt->execute()) {
    send_response(201, "success", "Transaksi Forex berhasil disimpan.");
} else {
