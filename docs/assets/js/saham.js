document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('saham-form');
    const tableBody = document.getElementById('saham-table-body');
    const type = 'saham';
    ...
    function renderTable(transactions) {
        if (transactions.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="6" class="text-center p-6 text-slate-500">Belum ada transaksi saham.</td></tr>';
            return;
        }
        tableBody.innerHTML = '';
        transactions.forEach(tx => {
            ...
            row.innerHTML = `
                ...
                <td class="p-3">${tx.lot_or_amount} Lot</td>
